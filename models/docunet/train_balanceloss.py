...
def train(args, model, train_features, dev_features, test_features):
    ...
    def finetune(features, optimizer, num_epoch, num_steps, model):
        ...
        ### Ege: Modified epoch count
        ### Original: train_iterator = [epoch + epoch_delta for epoch in range(num_epoch)]
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # if step ==0:
                    ...
                    if dev_score > best_score:
                        ...
                        with open("../predictions/docunet/docunet_dev_pred.json", "w") as fh:
                            json.dump(pred, fh)
                        if args.save_path != "":
                            torch.save({
                                'epoch': epoch,
...
def evaluate(args, model, features, tag="dev"):
    ...
    if len(ans) > 0:
        ...
        ### Original: best_f1, _, best_f1_ign, _, re_p, re_r = official_evaluate(ans, args.data_dir)
        best_f1, _, best_f1_ign, _, re_p, re_r = official_evaluate(ans, args.data_dir, args.save_path, args.load_path, args.test_file)
    output = {
        tag + "_F1": best_f1 * 100,
...
def main():
    ...
    print('args:',args)
    ### Ege: Change cuda:0 to cuda
    ### Original: device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    if args.load_path == "":  # Training
        ...
        with open("../predictions/docunet/docunet_predictions.json", "w") as fh:
            json.dump(pred, fh)
